...
`tests/e2e/ui/` is the per-role UI walkthrough (`superadmin` → `esteticista` → `admin`, ordered with project `dependencies`; `global.setup.ts` seeds the demo, creates `recepcion@nemedi.demo` if missing and cleans previous screenshots). Each chapter is one spec; each step records ✅/❌ plus a 1440×900 screenshot under `docs/manual/img/<rol>/capN-NN-<slug>.png` via the `Walk` helper in `walk.ts`, and results land in `tests/e2e/ui/.results/` (gitignored). Everything the walkthrough creates is deleted in `afterAll` (appointments in any state are reset to Agendada first, since only Agendada/Confirmada can be deleted). `docs/manual/MANUAL_USUARIO.md` is written from those results; the ❌ steps are listed in its "Pendientes" section.
