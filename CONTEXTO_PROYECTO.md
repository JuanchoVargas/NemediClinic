...
- Mi clínica (el SuperAdmin edita solo su tenant; su logo alimenta el branding), Sedes, Usuarios (Admin no puede crear usuarios: `register` exige SuperAdmin).
- Dashboard real (`GET /api/v1/dashboard`): citas de hoy por estado, pacientes activos, ingresos del mes, saldo pendiente, paquetes por vencer, stock en alerta, citas por día (14 d), top 5 procedimientos; filtro de sede para el dueño.
...
- Procedimientos (tarjetas con imagen), Paquetes, Paquetes de paciente (cierre automático al completar la última sesión, vencimiento por job diario, alerta "por vencer", eliminar asignación y pagos).
- Citas + Calendario + Hoja del día (no descuenta inventario).
- Inventario (productos con imagen, entradas, alertas, movimientos; no hay salidas ni consumo).
...
- Consumo de cabina: descuento de inventario al completar cita (no existe `Salida`)
- Borrar un paquete del catálogo deja inaccesibles sus asignaciones (warning EF 10622)
- Login acepta usuarios de tenants eliminados; pagos aceptan sobrepago; Admin ve "Nuevo usuario" pero recibe 403
...
- Tests unitarios (hoy 0; sí hay e2e: aislamiento multi-tenant, recorrido UI por rol y responsive)
