...
- Folder map: `src/api/*.api.ts` (one file per backend controller: TanStack Query hooks + axios calls), `src/pages/*.tsx` (dumb views; `admin/`, `super/` and `platform/` subfolders), `src/components/ui` (shadcn), `src/components/layout` (`Header`, `Footer`, `RootLayout`), `src/components/shared` (`PageContainer`, `PatientCombobox`, `FormDialog`, `SuspendedBanner`), `src/stores` (`auth.store.ts`, `toast.store.ts`), `src/lib` (`axios.ts`, `query-client.ts`, `utils.ts`), `src/types` (DTO mirrors), `src/styles` (`globals.css`, `tokens.css`).
